...
const DisplayAlbum = ({album}) => {
  ...
  if (!albumData) {
    return <p className="text-center text-xl mt-10">Album not found.</p>;
  }
  ...
  return albumData ? (
    <>
      ...
      <div className="mt-10 flex gap-8 flex-col md:flex-row md:items-end">
        <img className="w-48 rounded" src={albumData.image || ""} alt={albumData.name || "Album"} />
        <div className="flex flex-col">
          <p>Playlist</p>
          <h2 className="text-5xl font-bold mb-4 md:text-7xl">{albumData.name || "Unknown Album"}</h2>
          <h4>{albumData.desc || "No description available."}</h4>
          <p className="mt-1">
      ...
      {songsData.filter((item) => item.album === albumData.name).map((item, index) => (
          <div onClick={() => playWithId(item._id)} key={item._id} className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer">
            <p className="text-white">
              ...
              <img className="inline w-10 mr-5" src={item.image || ""} alt="Song Cover" />
              {item.name}
            </p>
